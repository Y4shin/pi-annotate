...
## 2026-08-14 — pi-annotate map finalized
The pi-annotate map delivered its single task (`pi-annotate-core`) and is complete: the extension provides both an agent-callable blocking `annotate` tool and a `/annotate` slash command that open a rendered, annotatable markdown view in a loopback browser, then feed the ephemeral annotations back into the agent context. All scope met; known follow-ups (in-place range highlight polish, real-browser-path verification) are recorded in the map's Known issues section.
