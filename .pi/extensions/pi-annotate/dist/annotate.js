(function() {
  ...
  function Annotate($$anchor, $$props) {
    ...
    let annotations = /* @__PURE__ */ state(proxy([]));
    ...
    async function loadDoc() {
      try {
        ...
        set(currentFile, data.path || "", true);
        document.title = "Annotate: " + get(currentFile);
        set(renderedHtml, renderMarkdown(data.markdown || ""), true);
      } catch (err) {
    ...
    function deleteAnnotation(created) {
      if (get(submitted)) return;
      set(annotations, get(annotations).filter((a) => a.created !== created), true);
    }
    function submit() {
      if (get(submitted)) return;
      set(submitted, true);
    ...
    function buildPayload() {
      return {
        ...
        annotations: get(annotations).slice()
      };
    }
    user_effect(() => {
